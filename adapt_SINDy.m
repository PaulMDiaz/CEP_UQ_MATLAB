function [Xi_SPGL1, Xi_SP, Xi_RR, Xi_IT, Sys_info] = adapt_SINDy(D,V,Lambda_cands)
...
for i = 1:length(Lambda_cands)+1
   if i == length(Lambda_cands)+1
    ...
    for l = 1:d
       ...
       try
           %tic; 
           xi_SPGL1 =  weights*SINDy(Phi_iter*Dbar,Phi_iter*V(:,l),'SPGL1');
           %time = toc;
       ...
       %tic;
       xi_SP =  weights*SINDy(Phi_iter*Dbar,Phi_iter*V(:,l),'SP');
       %time = toc;
       %fprintf('SP solved in = %0.3g seconds \n', time);
       %tic;
       xi_RR = weights*SINDy(Phi_iter*Dbar,Phi_iter*V(:,l),'RR');
       %time = toc;
       %fprintf('RR solved in = %0.3g seconds \n', time);
       %tic;
       xi_IT = weights*SINDy(Phi_iter*Dbar,Phi_iter*V(:,l),'IT');
       %time = toc;
